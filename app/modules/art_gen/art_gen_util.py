...
def select_random_elements(genres_list=None):
    if genres_list:
        all_genres = [
            record.genre_name for record in artgen_sql.query.filter(artgen_sql.genre_name.in_(genres_list)).all()
        ]
    else:
...
def generate_and_save_images(playlist_id, refresh=False, genre_list=None, quality="standard"):
    ...
    if refresh:
        last_entry = (
            artgenurl_sql.query.filter_by(playlist_id=playlist_id).order_by(artgenurl_sql.timestamp.desc()).first()
        )
        if not last_entry:
